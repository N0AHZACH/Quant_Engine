...
# --- 🎯 SWING SNIPER PARAMETERS (Top 5 Mode) 🎯 ---
...
def print_comprehensive_stats(daily_returns_list):
    ...
    print("\n=======================================================")
    print("📊 SWING SNIPER PERFORMANCE (Top 5)")
    print("=======================================================")
...
def run_walk_forward(device):
    print("\n--- PHASE 4: REALISTIC SWING VALIDATION (Top 5) ---")
    ...
    for f in tqdm(all_files, desc="Caching Data"):
        try:
            ...
            df['Realized_Ret_3D'] = df['Close'].pct_change(PREDICTION_HORIZON).shift(-PREDICTION_HORIZON).fillna(0)
        ...
        for d in test_dates:
            ...
            if len(candidates) > 0:
                ...
                for _, row in candidates.iterrows():
                    ...
                    if row['pred'] > 0:
                        actual_ret = row['ret']
                        if actual_ret < -(row['atr'] * 2.0): 
                            actual_ret = -(row['atr'] * 2.0)
